...
crimsonguard@htb[/htb]`$ man ascii`
...
crimsonguard@htb[/htb]`$ ./Arithmetic.sh  Addition: 10 + 10 = 20 Subtraction: 10 - 10 = 0 Multiplication: 10 * 10 = 100 Division: 10 / 10 = 1 Modulus: 10 % 4 = 2 Increase Variable: 2 Decrease Variable: 0`
...
crimsonguard@htb[/htb]`$ ./VarLength.sh  10`
...
crimsonguard@htb[/htb]`$ cat discovered_hosts.txt CIDR.txt  165.22.119.202 NetRange:       165.22.0.0 - 165.22.255.255 CIDR:           165.22.0.0/16`
...
crimsonguard@htb[/htb]`$ ./WhileBreaker.sh  Counter: 1 Counter: 2 Counter: 3 Counter: 4`
...
crimsonguard@htb[/htb]`$ ./PrintPars.sh  First parameter Second parameter Third parameter`
...
crimsonguard@htb[/htb]`$ ./Return.sh  Number of arguments: 0 Function status code: 1  Number of arguments: 1 Function status code: 0  Content of the variable:     Number of arguments: 1`
...
crimsonguard@htb[/htb]`$ bash -x -v CIDR.sh  #!/bin/bash  # Check for given argument if [ $# -eq 0 ] then 	echo -e "You need to specify the target domain.\n" 	echo -e "Usage:" 	echo -e "\t$0 <domain>" 	exit 1 else 	domain=$1 fi + '[' 0 -eq 0 ']' + echo -e 'You need to specify the target domain.\n' You need to specify the target domain.  + echo -e Usage: Usage: + echo -e '\tCIDR.sh <domain>' 	CIDR.sh <domain> + exit 1`

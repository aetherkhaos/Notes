...
|---|---|---|
![[Pasted image 20250203201813.png]]
...
![[Pasted image 20250203201857.png]]
...
![[Pasted image 20250203201947.png]]
...
![[Pasted image 20250203202205.png]]
...
#### Github
![[Pasted image 20250203202221.png]]

![[Pasted image 20250203202228.png]]
...
![[Pasted image 20250203202242.png]]
...
crimsonguard@htb[/htb]`$ sudo apt install vsftpd` 
...
crimsonguard@htb[/htb]`$ cat /etc/vsftpd.conf | grep -v "#"`
...
crimsonguard@htb[/htb]`$ cat /etc/ftpusers  guest john kevin`
...
```
crimsonguard@htb[/htb]$ ftp 10.129.14.136  
...
crimsonguard@htb[/htb]`$ ls | grep Notes.txt  'Important Notes.txt'`
...
crimsonguard@htb[/htb]`$ touch testupload.txt`
...
crimsonguard@htb[/htb]`$ sudo nmap --script-updatedb  Starting Nmap 7.80 ( https://nmap.org ) at 2021-09-19 13:49 CEST NSE: Updating rule database. NSE: Script Database updated successfully. Nmap done: 0 IP addresses (0 hosts up) scanned in 0.28 seconds`
...
crimsonguard@htb[/htb]`$ nc -nv 10.129.14.136 21`

crimsonguard@htb[/htb]`$ telnet 10.129.14.136 21`

It looks slightly different if the FTP server runs with TLS/SSL encryption. Because then we need a client that can handle TLS/SSL. For this, we can use the client `openssl` and communicate with the FTP server. The good thing about using `openssl` is that we can see the SSL certificate, which can also be helpful.

```
crimsonguard@htb[/htb]$ openssl s_client -connect 10.129.14.136:21 -starttls ftp  
...
what version of the FTP server is running on the target system? submit the entire banner as the answer.
```
┌──(kali㉿kali)-[~]
└─$ sudo nmap -sV -p21 -sC -A 10.129.202.5 --script-trace    
<snip>
NSE: TCP 10.10.14.3:47610 > 10.129.202.5:21 | CLOSE
NSOCK INFO [68.0630s] nsock_iod_delete(): nsock_iod_delete (IOD #17)
...
SF-Port21-TCP:V=7.95%I=7%D=2/3%Time=67A16EB2%P=x86_64-pc-linux-gnu%r(Gener
SF:icLines,74,"220\x20InFreight\x20FTP\x20v1\.1\r\n500\x20Invalid\x20comma
...
HOP RTT      ADDRESS
1   21.78 ms 10.10.14.1
2   21.92 ms 10.129.202.5
...
enumerate the FTP server and find the flag.txt file. submit the contents of the file.
...
150 Opening BINARY mode data connection for flag.txt (39 bytes)
    39      108.81 KiB/s 
226 Transfer complete
39 bytes received in 00:00 (1.23 KiB/s)
ftp> bye
221 Goodbye.
...
```
crimsonguard@htb[/htb]`$ cat /etc/samba/smb.conf | grep -v "#\|\;"   
[global]
...
```
crimsonguard@htb[/htb]`$ smbclient -N -L //10.129.14.128
...
```
crimsonguard@htb[/htb]`$ smbclient //10.129.14.128/notes  
...
```
crimsonguard@htb[/htb]$ sudo nmap 10.129.14.128 -sV -sC -p139,445  
...
```
crimsonguard@htb[/htb]$ rpcclient -U "" 10.129.14.128  
...
```
crimsonguard@htb[/htb]`$ for i in $(seq 500 1100);do rpcclient -N -U "" 10.129.14.128 -c "queryuser 0x$(printf '%x\n' $i)" | grep "User Name\|user_rid\|group_rid" && echo "";done          
...
```
crimsonguard@htb[/htb]`$ samrdump.py 10.129.14.128  
...
```
crimsonguard@htb[/htb]`$ smbmap -H 10.129.14.128  
...
```
crimsonguard@htb[/htb]$ crackmapexec smb 10.129.14.128 --shares -u '' -p ''  
...
```
crimsonguard@htb[/htb]`$ git clone https://github.com/cddmp/enum4linux-ng.git 
$ cd enum4linux-ng 
...
```
crimsonguard@htb[/htb]`$ ./enum4linux-ng.py 10.129.14.128 -A  ENUM4LINUX - next generation   
...
domain_password_information:   
	...
	min_pw_length: 5   min_pw_age: none   
